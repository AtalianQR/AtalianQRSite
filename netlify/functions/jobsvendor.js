// === netlify/functions/jobsvendor.js (Met VendorId 000016 override + melder-fix) ===
...
async function handlePostAction(event, body, cfg) {
  ...
  if (!vendorId) {
    ...
    // Geen vendor aan de job gekoppeld → melder-scenario
    // → ADD_JOB_DOC ALTIJD toestaan, zodat foto's altijd kunnen worden opgeladen.
    if (isDocAction) {
      allowed = true;
      console.warn("[jobsvendor][AUTH] no vendorId → treating as melder upload, allowing ADD_JOB_DOC", {
        jobId,
        email
      });
    } else {
  ...
  // 4) Actie uitvoeren (ADD_JOB_DOC / ADD_INFO / CLOSE)
  if (isDocAction) {

    ...
    if (url.pathname === "/api/log") {
      ...
      if (request.method === "GET") {
        const limit = Math.min(
          50000,
          Math.max(1, parseInt(url.searchParams.get("limit") || "5000", 10))
        );
        ...
        while (keys.length < limit) {
          const page = await env.atalian_logs.list({
            prefix: "events/",
            cursor,
            limit: 200, // << hier zat je fout, geen "..."
          });
        ...
        for (const name of keys) {
          ...
          if (val && (!since || (val.server_ts || val.ts || 0) >= since)) {
            items.push(val);
          }
        }
